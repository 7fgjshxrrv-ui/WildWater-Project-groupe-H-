...
PtrNodeAVL rotationGauche(PtrNodeAVL a) {
    PtrNodeAVL pivot = a->right;  // Le pivot est le fils droit
    a->right = pivot->left;       // Le fils gauche du pivot devient le fils droit de a
...
PtrNodeAVL rotationDoubleDroite(PtrNodeAVL a) {
    // 1. Rotation Gauche sur le fils gauche
    a->left = rotationGauche(a->left);
    // 2. Rotation Droite sur le noeud courant
    return rotationDroite(a);
}

PtrNodeAVL rotationDoubleGauche(PtrNodeAVL a) {
    // 1. Rotation Droite sur le fils droit
    a->right = rotationDroite(a->right);
    // 2. Rotation Gauche sur le noeud courant
